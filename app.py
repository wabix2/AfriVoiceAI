...
# ---------------- Database Setup ----------------
DATABASE = 'instance/afrivoice.db'
...
for name, data in kb.items():
    # Combine topic name and keywords into a single document
    doc = name + " " + " ".join(data.get("keywords", []))
    topic_texts.append(doc.lower())
...
# ---------------- Smart Topic Matching (Semantic + Keyword) ----------------
def find_best_topic(query):
    query_lower = query.lower()
    if tfidf_matrix is not None and len(topic_texts) > 0:
        query_vec = vectorizer.transform([query_lower])
        similarities = cosine_similarity(query_vec, tfidf_matrix).flatten()
        best_idx = np.argmax(similarities)
        best_score = similarities[best_idx]
        if best_score > 0.2:  # threshold
            return kb[topic_names[best_idx]], best_score
...
def generate_response(query, user_id=None):
    ...
    # Check if it's a follow-up referring to last topic
    ...
    if data and score > 0:
        ...
        if original_lang == "am":
            ...
            response = "Dhiifama, waa'ee gaaffii kanaa odeeffannoo hin qabnu. Waa'ee xaafee facaasuu, hoolota gabbisuu, damma sassaabuu ykn fayyaa gaafadhaa."
        else:
...
<body class="bg-gradient-to-br from-emerald-50 via-teal-50 to-green-100 min-h-screen flex items-center justify-center">
  <div class="max-w-md w-full mx-4">
    <div class="bg-white rounded-2xl shadow-xl p-8">
      ...
      <p class="mt-6 text-center text-gray-600">Don't have an account? <a href="{{ url_for('signup') }}" class="text-emerald-700 font-medium hover:underline">Create one</a></p>
    </div>
  </div>
</body>
</html>
...
<body class="bg-gradient-to-br from-emerald-50 via-teal-50 to-green-100 min-h-screen flex items-center justify-center">
  <div class="max-w-md w-full mx-4">
    <div class="bg-white rounded-2xl shadow-xl p-8">
      ...
      <p class="mt-6 text-center text-gray-600">Already have an account? <a href="{{ url_for('login') }}" class="text-emerald-700 font-medium hover:underline">Sign in</a></p>
    </div>
  </div>
</body>
</html>
...
<body class="bg-gradient-to-b from-emerald-50 to-teal-100 min-h-screen flex">
  <!-- Sidebar -->
  <div class="w-80 bg-white/90 backdrop-blur-sm border-r border-emerald-200 p-4 flex flex-col hidden md:flex">
  ...
  <div class="flex-1 flex flex-col">
    <header class="bg-white/80 backdrop-blur-sm p-4 border-b border-emerald-200 flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-xl font-semibold text-gray-800">AfriVoice AI</h1>
        <span class="text-xs bg-emerald-100 text-emerald-800 px-2 py-1 rounded-full">Your Personal Assistant</span>
      </div>
    </header>
    ...
    async function loadHistory() {
      try {
        ...
        data.slice(0, 10).forEach(item => {
          ...
          div.className = 'p-2 hover:bg-gray-100 rounded-lg cursor-pointer text-sm';
          div.textContent = item.message.substring(0, 30) + '...';
